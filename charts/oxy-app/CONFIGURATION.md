...
| ingress.enabled | bool | `false` | Enable creating an Ingress resource for the app |
| ingress.ingressClassName | string | `""` | Optional IngressClass name to use |
| ingress.annotations | object | `{}` | Annotations to add to the Ingress metadata |
| ingress.path | string | `/` | Default path for hosts that don't specify paths |
| ingress.pathType | string | `Prefix` | Path type for ingress paths |
| ingress.hosts | list | `[{host: chart-example.local, paths: []}]` | List of hosts and optional per-host paths |
| ingress.tls | list | `[]` | TLS entries for the Ingress (hosts + secretName) |
...
The chart exposes an optional Ingress resource controlled by the `ingress` values.

Example enabling an ingress with a host and TLS:
